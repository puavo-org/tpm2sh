...
use crate::{PipelineObject, TpmError};
...
/// # Errors
///
/// Returns a `TpmError` if the URI is malformed, a file cannot be read,
...
pub fn resolve_uri_to_bytes(
    ...
    if let Some(path) = uri_str.strip_prefix("file://") {
        std::fs::read(Path::new(path)).map_err(|e| TpmError::File(path.to_string(), e))
    } else if let Some(data) = uri_str.strip_prefix("data://") {
        let (encoding, value) = data
            .split_once(',')
            .ok_or_else(|| TpmError::Parse(format!("Invalid data URI format: '{uri_str}'")))?;
        match encoding {
            "utf8" => Ok(value.as_bytes().to_vec()),
            "hex" => hex::decode(value).map_err(TpmError::from),
            "base64" => base64_engine.decode(value).map_err(TpmError::from),
        ...
        let actual_index = if index < 0 {
            let offset = usize::try_from(index.abs())
                .map_err(|_| TpmError::Parse("Invalid negative index".to_string()))?;
        ...
        let obj = pipeline_objects
            .get(idx)
            .ok_or_else(|| TpmError::Parse(format!("Pipeline index out of bounds: {index}")))?;
...
/// # Errors
///
/// Returns a `TpmError` if the URI is malformed or the handle is not a valid hex u32.
pub fn parse_tpm_handle_from_uri(uri_str: &str) -> Result<u32, TpmError> {
