...
fn test_subcommand_create_primary(test_context: TestFixture) {
    let create_cmd = Command::CreatePrimary(CreatePrimary {
...
fn test_subcommand_load_import(test_context: TestFixture) {
    ...
    let load_cmd = Command::Load(Load {
...
fn test_subcommand_pcr_event(test_context: TestFixture) {
    ...
    let pcr_event_cmd = Command::PcrEvent(PcrEvent {
    ...
    let pcr_policy_cmd = Command::Policy(Policy {
...
fn test_subcommand_policy_default_is_pcr_read(test_context: TestFixture) {
    let policy_cmd = Command::Policy(Policy {
...
fn test_subcommand_seal_unseal(test_context: TestFixture) -> Result<()> {
    ...
    let mut parent_context_uri_buf = Vec::new();
    let mut context = Context::new(&mut parent_context_uri_buf);
    create_cmd.run(Some(&test_context.device), &mut context)?;
    ...
    let seal_cmd = Command::Seal(Seal {
    ...
    seal_cmd.run(Some(&test_context.device), &mut context)?;
    ...
    let unseal_cmd = Command::Load(Load {
        parent: parent_uri,
    ...
    unseal_cmd.run(Some(&test_context.device), &mut context)?;
...
fn test_subcommand_seal_unseal_policy(test_context: TestFixture) -> Result<()> {
    ...
    let start_session_cmd = Command::StartSession(StartSession::default());
    let mut session_uri_buf = Vec::new();
    let mut context = Context::new(&mut session_uri_buf);
    start_session_cmd.run(Some(&test_context.device), &mut context)?;
